...
void ui_manager_next()
{
    ...
    // Only queue one pending transition at a time
...
void ui_manager_init()
{
    ...
    // Initialize screens array with dynamic engine screens
    init_screens_array();
...
void ui_manager_update()
{
    ...
    // Only update the current screen
...
void ui_manager_process_deferred_screen_creation()
{
    ...
    if (pending_create_screen_index >= 0 &&
        pending_create_screen_index < screen_count) {

        create_if_needed(
            screens[pending_create_screen_index]
        );

        pending_create_screen_index = -1;
    }

    // Queue the load if there is a pending transition
    if (pending_screen_index >= 0 &&
...
//
// IMPORTANT:
// This function is called from ui_manager_process_deferred_screen_creation()
// while the LVGL lock is already held.
// -----------------------------------------------------------------------------
...
void ui_manager_reinit_screens()
{
    ...
    // Destroy all existing screens before rebuilding the array
    for (int i = 0; i < old_screen_count; i++) {
    ...
    // Reinitialize screens array using current configuration
    init_screens_array();
