...
static void lv_engine_display(lv_obj_t *parent)
{
    ...
    engine_rpm_indic = lv_meter_add_needle_line(
        engine_rpm_meter,
        scale,
        4,
        lv_palette_main(LV_PALETTE_GREY),
    ...
    eng_temp_indic = lv_meter_add_arc(
        eng_temp_meter,
        eng_temp_scale,
        3,
...
static void engine_update_cb(void)
{
    char buf[64];

    set_engine_rpm_value(
        engine_rpm_indic,
    ...
    if (fresh(shipDataModel.navigation.speed_over_ground.age))
        snprintf(buf, sizeof(buf), "SOG (kt):\n%.1f",
                 shipDataModel.navigation.speed_over_ground.kn);
    else
        snprintf(buf, sizeof(buf), "SOG (kt):\n--");

    lv_label_set_text(eng_sog_label, buf);


    if (fresh(shipDataModel.propulsion.engines[0].alternator_voltage.age))
        snprintf(buf, sizeof(buf), "ALT (V):\n%.1f",
                 shipDataModel.propulsion.engines[0].alternator_voltage.volt);
    else
        snprintf(buf, sizeof(buf), "ALT (V):\n--");

    lv_label_set_text(eng_alternator_label, buf);
}
