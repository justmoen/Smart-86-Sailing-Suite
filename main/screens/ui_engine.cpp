...
// -----------------------------------------------------------------------------
// Engine gauge update rates
// -----------------------------------------------------------------------------

// RPM needle: 10 Hz
#define RPM_NEEDLE_UPDATE_MS 100

// Everything below: 5 Hz
#define ENGINE_VALUE_UPDATE_MS 200
...
struct EngineScreenState {
    ...
    // Last known values
    uint32_t last_rpm_update_ms = 0;
    uint32_t last_value_update_ms = 0;
...
)
{
    ...
    // -------------------------------------------------------------------------
    state->engine_rpm_scale =
        lv_scale_create(scr->screen);
    ...
    // Stop the scale layout engine from flattening the needle object down to 0x0 size
    lv_obj_set_layout(
        state->engine_rpm_needle,
        LV_LAYOUT_NONE
    );
    ...
    int temp_x =
        config.engine_oil_pressure_enabled
            ? 125
            : 0;
...
)
{
    ...
    // RPM needle: 10 Hz
    //
    if ((now - state->last_rpm_update_ms) >=
        RPM_NEEDLE_UPDATE_MS)
    {
        state->last_rpm_update_ms =
            now;
    ...
    // RPM numeric label
    //
    if ((now - state->last_value_update_ms) >=
        ENGINE_VALUE_UPDATE_MS)
    {
        state->last_value_update_ms =
            now;
...
)
{
    ...
    for (int i = 0; i < num_engines; i++)
    {
        ...
        if (i == 0)
            engine_id = config.engine_screen_1_id;
        else if (i == 1)
            engine_id = config.engine_screen_2_id;
        else
            engine_id = i;
